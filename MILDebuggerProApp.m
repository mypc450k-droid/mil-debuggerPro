    ...
    methods (Access=private)
        ...
        function runMIL(app)
            ...
            try
                ...
                captureMap=app.Core.Logging.getCaptureMap();
                app.Core.Session.create(app.ModelName,app.Inventory,cr,sr,captureMap);
                loggedCount=numel(app.Core.Session.Session.Signals);
                cov=app.Core.Logging.getCoverage();
                app.SessionLabel.Text=sprintf('Cached session: %s | %.3fs | %d signals | capture %d/%d', ...
                    datestr(app.Core.Session.Session.CreatedAt,'yyyy-mm-dd HH:MM:SS'), ...
                    sr.Elapsed,loggedCount,cov.Enabled,cov.Requested);

                app.refreshSignalList();
                if loggedCount==0
                    app.setStatus(sprintf(['MIL completed, but logsout contains 0 usable signals. ' ...
                        'Capture requested %d and enabled %d. Check model logging limitations or diagnostics.'], ...
                        cov.Requested,cov.Enabled));
                else
                    app.setStatus(sprintf(['MIL complete. Cached %d signals. Capture coverage: %d/%d enabled, %d failed. ' ...
                        'Block I/O now uses this cached run and will not simulate again.'], ...
                        loggedCount,cov.Enabled,cov.Requested,cov.Failed));
                end
                cov=app.Core.Logging.getCoverage();
